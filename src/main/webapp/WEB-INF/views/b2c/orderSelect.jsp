...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			...
			$(function() {
				$('#orderTable').bootstrapTable({
					url:"${pageContext.request.contextPath}/order/selAll",
					queryParamsType : '',
					pagination: true,
		...
		//日期選擇框
			...
			/* function operateFormatter(e, value, row, index) {
				return [
				    '<a class="btn btn-primary see" role="button"  href="javascript:void(0)">查看',
				      '<i class="fa fa-eye"></i>',
				      '</a>  '
				    ].join('')
   ...
   //查看按钮发送请求
       ...
       function operateFormatter(value, row, index) {
		    return [
		      '<a class="btn btn-primary like" onclick="goorderuserinfo('+row.orderid+')" role="button"  href="javascript:void(0)">选择',
		      '</a>  ',
		    ].join('')
		  }
       function goorderuserinfo(orderid){
			
			$.post("/CRM/order/getoudetail",{"orderId":orderid},function(data){
				console.log(orderid);
				location.href="/CRM/order/sodetail"
			},"json");

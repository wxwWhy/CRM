...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$('#orderTable').bootstrapTable({
					url:"${pageContext.request.contextPath}/shop/selAll",
					queryParamsType : '',
					pagination: true,
		...
		//日期選擇框
			...
			function actionFormatter(value, row, index) {
				var id = value;
				var result = "<button type='button' class='btn btn-success'   onclick='look("+id+")'>查看</button>";
				return result;
			...
			//先判断是否为空，不为空的话就取值
				$(".selOrder").each(function() {
					...
					if(temp!=$("#commodityList").val()){
						params.commoditylist = $("#commodityList").val();
					}
			//获取下拉框中的值
	...
	<body>
		...
		<div class="container">
			<h6 id="h6">B2C销售管理-->B2C网店订单订单管理-->网店订单列表</h6>
          ...
          <form id="queryForm">
				...
				订单类型：<select id="id1"   class="selOrder" >
				 ...
				 下单日期期间：<input type="text"  id="test1"    class="selOrder"/>
				  ...
				  商品名称：<input type="text"  id="commodityList"    class="selOrder"/>
				...
				<input type="submit"  id="dao" class="btn btn-primary" value="网店订单导入"></button>
			</form>
			<table id="orderTable"> 
